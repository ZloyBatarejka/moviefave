...
export const searchApiHandler = (title: string, page: number) => {
    return async (dispatch: ThunkDispatch<IInitialSearchState, undefined, ISearchAction>) => {
        ...
        try {
            ...
            const searchResult: IMovieSearchCard[] = response.data.results.map((item: any) => {
...
export const auth = (email: string, password: string, isLogin: boolean) => {
    return async (dispatch: ThunkDispatch<IInitialAuthState, undefined, ILogin | IModalAction>) => {
        const authData = {
            email,
            password,
...
export const setFavorites = (url: string) => {
    return async (dispatch: ThunkDispatch<IInitialFaveState, undefined, IFaveAction | IFaveArrayAction>) => {
        const movies = await axios.get(`https://moviefave-56a11.firebaseio.com/${url}.json`);
...
export const removeFavotire = (url: string, movieUrl: string | null, movies: IMovieSearchCard[]) => {
    return async (dispatch: ThunkDispatch<IInitialFaveState, undefined, IFaveArrayAction>) => {
        await axios.delete(`https://moviefave-56a11.firebaseio.com/${url}/${movieUrl}.json`);
        dispatch({
            type: REMOVE_FAVE,
            payload: movies,
        });
...
export const setMovie = (id: number) => {
    return async (dispatch: ThunkDispatch<IInitialMovieState, undefined, IMovieAction>) => {
        const response = await axios.get(
...
export const sort = (movies: IMovieSearchCard[], sortType: string) => {
    return (dispatch: ThunkDispatch<IInitialSearchState, undefined, ISearchAction | ISort>) => {
        if (sortType === "rating") {
